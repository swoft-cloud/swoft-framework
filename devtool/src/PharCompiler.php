...
class PharCompiler
{
    ...
    public function findChangedByGit()
    {
        ...
        list(, $output, ) = ProcessHelper::run('git status -s -u', $this->basePath);
    ...
    {
        return DirHelper::directoryIterator(
            $directory,
...
EOF;
    ...
    private function collectInformation()
    {
        ...
        $basePath = $this->basePath;
        list($code, $ret,) = ProcessHelper::run('git log --pretty="%H" -n1 HEAD', $basePath);
        ...
        list($code, $ret,) = ProcessHelper::run('git log -n1 --pretty=%ci HEAD', $basePath);
        ...
        // 获取到最新的 tag
        list($code, $ret,) = ProcessHelper::run('git describe --tags --exact-match HEAD', $basePath);
        if ($code === 0) {
            $this->version = trim($ret);
        } else {
            list($code, $ret,) = ProcessHelper::run('git branch', $basePath);
